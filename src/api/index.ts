...
export const GetUserData = async (userName: string): Promise<Data> => {
    try {
        ...
        const repos: Repository[] = await axiosInstance.get(`${userName}/repos`).then(res => res?.data.filter((r: Repository) => !r.fork))
        ...
        const stars: Repository[] = await axiosInstance.get(`${userName}/starred`).then(res => res?.data)
